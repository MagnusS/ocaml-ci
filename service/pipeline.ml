  ...
  let specs =
    ...
    | Ok analysis ->
      ...
      | `Opam_build selections ->
        let lint_platform = List.hd selections in
        let builds =
        ...
        and lint =
          [
            Build.Spec.opam ~label:"(lint-fmt)" ~selection:lint_platform ~analysis (`Lint `Fmt);
            Build.Spec.opam ~label:"(lint-doc)" ~selection:lint_platform ~analysis (`Lint `Doc);
          ]
        in
        lint @ builds
  in
